...
import {StyleSheet,StatusBar,View,ScrollView,Image,Text,Animated,TouchableWithoutFeedback} from "react-native";
...
export default class personIndex extends React.Component {
  ...
  render() {
    return (
      <View style={{ flex: 1 }}>
        <ScrollView>
          <Image source={require("../img/timg.jpg")} style={styles.perosonBG} />
          <View style={styles.action}>
            <TouchableWithoutFeedback onPress={() => this.choosePhoto()}>
              <Image
          ...
          <View style={styles.name}>
            ...
            <Image
              style={styles.name_2}
              source={require("../img/female_03.png")}
            />
          </View>
          <View style={styles.favo}>
            <Text>关注 40</Text>
            <View style={styles.favoBorder} />
            <Text>粉丝 11440</Text>
          </View>
          <Text
            style={styles.selfIntroduce}
            numberOfLines={1}
            ellipsizeMode={"tail"}
          >
            备兵驯马以待战机，金鳞岂是池中物，权且忍让,非我族类，其心必异非我族类，其心必异
          </Text>
          <Text
            style={styles.seeMore}
            onPress={() => this.props.navigation.navigate("allInfo")}
          >
            查看更多资料
          </Text>
          ...
          <View style={styles.active}>
            <Text
              style={{ fontSize: 0.045 * vw, color: this.state.activeColor_1 }}
              onPress={() => this._changeTab_1()}
            >
              动态
            </Text>
            <Text
              style={{ fontSize: 0.045 * vw, color: this.state.activeColor_2 }}
              onPress={() => this._changeTab_2()}
            >
              相册
            </Text>
            <Animated.View
        ...
        <BottomBack navigation={this.props.navigation} />
      </View>
    );
  }
  _changeTab_1() {
    this.setState({ activeColor_2: "#888", activeColor_1: "#333" });
    Animated.timing(this.state.tabLeft, {
      toValue: 0.2 * vw,
      duration: 300
    }).start();
  }
  ...
  choosePhoto() {
    ...
    ImagePicker.showImagePicker(options, response => {
      ...
      if (response.didCancel) {
        ...
        _this.props.navigation.navigate('imageCrop',{"uri":response.uri})
      }
    });
  }
}
